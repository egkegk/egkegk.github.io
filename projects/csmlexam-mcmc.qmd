...
\begin{algorithm}
  \caption{Parallel Tempering Algorithm}
  \begin{algorithmic}
    \Procedure{ParallelTempering}{$n, \mathbf{T}, \sigma^{\mathbf{2}}, \mathbf{X_0}$}
      \For{$i = 1, 2, \dots, n$}
        \Procedure{X-Step}{}
          \For{$j = 1,2,\dots,M$}
            \State generate $Y \sim N(X_{i-1, j}, \sigma_{j}^2)$
            \State generate $u \sim U([0,1])$
            \If{$u \leq \left(\frac{f(Y)}{f(X_{i-1,j})}\right)^{\frac{1}{T_j}}}$
              \State $X_{i,j} = Y$
            \Else
              \State $X_{i,j} = X_{i-1,j}$
            \EndIf
          \EndFor
        \EndProcedure
        \Procedure{Swap}{}
          \State generate $m=$ random integer on the interval $[1, M - 1]$
          \State generate $u'\sim U([0,1])$
          \If{$u \leq \frac{f(X_{i,m})^{1/T_{m+1}}f(X_{i,m+1})^{1/T_{m}}}{f(X_{i,m})^{1/T_m}f(X_{i,m+1})^{1/T_{m+1}}}$}
            \State exchange $X_{i,m}$ and $X_{i,m+1}$
          \EndIf
        \EndProcedure
      \EndFor
    \EndProcedure
